...
fn _validate_token_sign_and_exp(origin_token: &Token, salt: &str, key: &[u8]) -> Result<()> {
    // -- Validate signature
    let new_sign_b64u = _token_sing_into_b64u(&origin_token.ident, &origin_token.exp, salt, key)?;
...
fn _token_sing_into_b64u(ident: &str, exp: &str, salt: &str, key: &[u8]) -> Result<String> {
...
mod tests {
    ...
    #[test]
    fn test_web_token_err_expired() -> Result<()> {
        // -- Setup & Fixtures
        let fx_user = "user_one";
        let fx_salt = "pepper";
